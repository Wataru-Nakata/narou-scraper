...
def retrieve_text(ncode,pbar):
    ...
    try:
        ...
        pbar.set_description('Error occured while requesting', info_url)
        raise ValueError
    honbun = ''
...
if __name__=="__main__":
    ...
    for idx, row in pbar:
        if os.path.exists(os.path.join('data',row['ncode']+'.txt')):
            continue
        try:
